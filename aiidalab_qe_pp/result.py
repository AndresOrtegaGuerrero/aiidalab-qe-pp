...
def process_orbitals(data):
    ...
    kpoint_band_data = [{'kpoint': kpoint, 'bands': sorted(list(bands))} for kpoint, bands in kpoint_band_dict.items()]
...
class Result(ResultPanel):
    ...
    def _on_tab_change(self, change):
        ...
        if selected_index is not None:
            for viewer_var in ["viewer_charge_dens", "viewer_spin_dens", "viewer_wfn", "viewer_ildos", "viewer_stm"]:
                result_viewer = getattr(self, viewer_var, None)
                if result_viewer:
                    result_viewer.viewer._widget.send_js_task({"name": "tjs.onWindowResize", "kwargs": {}})
